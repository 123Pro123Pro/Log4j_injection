...
public class BurpExtender implements IBurpExtender {
    private static final String name = "Log4j Injection";
    ...
    private void processInteraction(IBurpCollaboratorInteraction interaction) {
        ...
        */
        ...
        Utilities.callbacks.addScanIssue(
                new CustomScanIssue(req.getHttpService(), reqInfo.getUrl(), new IHttpRequestResponse[]{req}, "Log4j Pingback ("+interaction.getProperty("type")+"): "+type, message+interaction.getProperties().toString(), severity, "Certain", "Panic"));
    }
...
class Correlator {
    ...
    Correlator() {
        ...
        try {
            ...
            Utilities.out("The Collaborator appears to be misconfigured. Please run a health check via Project Options->Misc. Also, note that Log4j Injection Tool does not support the IP-address mode.");
        }
...
class Injector implements IProxyListener {
    ...
    public byte[] injectPayloads(byte[] request, Integer requestCode) {
	    ...
	    payload = payload.replace("%h", Utilities.getHeader(request, "Host").replace(":", "_"));
            switch ( injection[0] ){
                case "param":
